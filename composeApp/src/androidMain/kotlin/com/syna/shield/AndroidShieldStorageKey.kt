...
actual object ShieldStorageKey {
    ...
    actual fun decrypt(payload: ByteArray): ByteArray? {
        ...
        // native 心跳校验（JVM hook 免疫）：仅当 Shield 启用（引擎在驱动 native 槽）时
        // 校验——Shield 关闭后引擎停止，native 槽自然过期，不得锁死数据
        if (ShieldGate.isArmed() && NativeShield.loaded && NativeShield.gateFresh() == 0) return null
